...
| `loadError` 상태 | 추첨일 | 코드 | 배너 색 |
|---|---|---|---|
| `"NO_DATA"` | — | `ERR-NO-DATA` | 빨강 |
| `"FETCH_FAILED"` | — | `ERR-FETCH-FAILED` | 빨강 |
| `null` | 8일+ 경과 | `ERR-STALE-DATA · Nd` | 노랑 |
| `null` | ≤7일 | (없음, 배너 숨김) | — |

사용자 문구는 기술 용어 없이 "뭘 해야 하는지"만 안내합니다. 에러코드는 배너 우측 하단에 작게 `[ERR-...]` 형태로 붙어서 개발자만 보면 됩니다.

상세·대응은 [OPERATIONS.md](OPERATIONS.md#에러코드-레퍼런스-사용자-배너) 참조.
